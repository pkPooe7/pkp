...
export class ClassificationDeletePopupComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.activatedRoute.data.subscribe(({ classification }) => {
      setTimeout(() => {
        this.ngbModalRef = this.modalService.open(ClassificationDeleteDialogComponent as Component, { size: 'lg', backdrop: 'static' });
        this.ngbModalRef.componentInstance.classification = classification;
        this.ngbModalRef.result.then(
          result => {
            this.router.navigate(['/classification', { outlets: { popup: null } }]);
